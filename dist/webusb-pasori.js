...
(function (Scratch) {
  ...
  var block_definitions_default = {
  	extensionName: "WebUSB PaSoRi",
  	blocks: [
  		{
  			"opcode": "connectPasoriBlock",
  			"blockType": "COMMAND",
  			"text": "connect PaSoRi as [READER_ID]",
  			"description": "Requests permission and connects a Sony PaSoRi reader over WebUSB with a project-local reader name.",
  			"arguments": { "READER_ID": {
  				"type": "STRING",
  		...
  		},
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "wait for NFC card on PaSoRi [READER_ID] set runtime var [RUNTIME_VAR] to IDm",
  			"description": "Waits until a card is read on the named PaSoRi and stores its IDm in a runtime variable.",
  			"arguments": {
  				"READER_ID": {
  					"type": "STRING",
  					"defaultValue": "default"
  		...
  		},
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "wait for NFC card on PaSoRi [READER_ID] set runtime var [RUNTIME_VAR] to IDm and broadcast [MESSAGE]",
  			"description": "Waits until a card is read on the named PaSoRi, stores its IDm, and broadcasts a message.",
  			"arguments": {
  				"READER_ID": {
  					"type": "STRING",
  					"defaultValue": "default"
  		...
  		},
  		{
  			...
  			"blockType": "REPORTER",
  			"text": "last NFC IDm on PaSoRi [READER_ID]",
  			"description": "Returns the most recent NFC IDm read by the named PaSoRi.",
  			"arguments": { "READER_ID": {
  				"type": "STRING",
  				"defaultValue": "default"
  			} }
  ...
  var sonyVendorId = 1356;
  var ack = Object.freeze([
  	0,
  	0,
  	255,
  	0,
  ...
  var defaultReaderId = "default";
  function hex(bytes) {
  	return bytes.map((value) => value.toString(16).padStart(2, "0")).join("").toUpperCase();
  }
  function abortError() {
  	const error = /* @__PURE__ */ new Error("NFC reading was aborted.");
  	error.name = "AbortError";
  	return error;
  }
  function webUsb() {
  	const usb = globalThis.navigator?.usb;
  	if (!usb || typeof usb.requestDevice !== "function") throw new Error("WebUSB PaSoRi requires navigator.usb.requestDevice.");
  	return usb;
  }
  function normalizeReaderId(value) {
  	return String(value ?? "").trim() || defaultReaderId;
  }
  var PasoriDevice = class {
  	constructor(device) {
  		this.device = device;
  	}
  	async release() {
  	...
  	async receive(length) {
  		const view = (await this.device.transferIn(1, length)).data;
  		if (!view) return [];
  	...
  	async getType2TagInfo() {
  		...
  		const idm = hex((await this.sendCommand(4, [
  			54,
  			1,
  			147,
  			32
  		])).slice(15, 19));
  		return idm === "00" || idm.length === 0 ? null : {
  			spec: "Type2",
  			idm,
  			pmm: null
  		};
  	...
  	async getType3TagInfo() {
  		...
  		const data = await this.sendCommand(4, [
  			110,
  			0,
  			6,
  			0,
  		...
  		return {
  			spec: "Type3",
  			idm,
  			pmm: hex(data.slice(25, 33))
  		};
  	}
  };
  var WebUsbPasoriExtension = class {
  	constructor() {
  	...
  	writeRuntimeVariable(name, value) {
  		...
  		if (!temporaryVariables || typeof temporaryVariables.setRuntimeVariable !== "function") throw new Error("Temporary Variables (lmsTempVars2) must be loaded before using WebUSB PaSoRi blocks.");
